    ...
    if (!Number.isNaN(n) && n >= 0) {
        ...
        if (lastQuestion === "professional" && pros?.length) {
            if (n === 0) {
                ...
                } else if (lastQuestion === "professional") {
                    // Handle Professional Selection by Number
                    const offeredProIds = stateRow?.state?.last_offer?.professional_options;

                    if (offeredProIds && !isNaN(n) && n >= 0 && n <= offeredProIds.length) {
                        // 0 = Any/First Available (We treat as "First" for now or handle as specific flag)
                        // The list usually starts at 1 for specific pros.
                        // 0 is "Primeiro disponível". Logic: if 0, pick first or random? 
                        // Let's assume 0 = Auto-assign (First in list).

                        let pickedId: string | undefined;

                        if (n === 0) {
                            // 0 = Any. We pick the first one as default or handle "ANY" logic. 
                            // For simplicity, let's pick the first one visible.
                            pickedId = offeredProIds[0];
                        } else {
                            pickedId = offeredProIds[n - 1];
                        }

                        if (pickedId) {
                            const pickedPro = pros?.find(p => p.id === pickedId);
                            if (pickedPro) {
